...
class CustomHJBLQ(HJBLQ):
    """
    Customizable Multidimensional HJBLQ PDE 
    with parameterized terminal condition g.
    """
    def __init__(self, eqn_config):
        super(CustomHJBLQ, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 动态参数，用于调整终止条件中的参数

    def g_tf(self, t, x):
        return tf.math.log((1 + self.g_param * tf.reduce_sum(tf.square(x), 1, keepdims=True)) / 2)
...
class CustomAllenCahn(AllenCahn):
    """
    Customizable Multidimensional AllenCahn PDE 
    with parameterized terminal condition g.
    """
    def __init__(self, eqn_config):
        super(CustomAllenCahn, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 动态参数，用于调整终止条件中的参数

    def g_tf(self, t, x):
        return (0.5 * self.g_param) / (1 + 0.2 * tf.reduce_sum(tf.square(x), 1, keepdims=True))
...
class CustomPricingDefaultRisk(PricingDefaultRisk):
    """
    Customizable Nonlinear Black-Scholes equation with default risk.
    """
    def __init__(self, eqn_config):
        super(CustomPricingDefaultRisk, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 默认参数为1.0

    def g_tf(self, t, x):
        return tf.reduce_min(x, 1, keepdims=True) * self.g_param  # 引入可调参数
...
class CustomPricingDiffRate(PricingDiffRate):
    """
    Customizable Nonlinear Black-Scholes equation with different interest rates.
    """
    def __init__(self, eqn_config):
        super(CustomPricingDiffRate, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 默认参数为1.0

    def g_tf(self, t, x):
        temp = tf.reduce_max(x, 1, keepdims=True)
        return (tf.maximum(temp - 120, 0) - 2.0 * tf.maximum(temp - 150, 0)) * self.g_param  # 引入可调参数
...
class CustomBurgersType(BurgersType):
    """
    Customizable Multidimensional Burgers-type PDE 
    with parameterized terminal condition g.
    """
    def __init__(self, eqn_config):
        super(CustomBurgersType, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 动态参数，用于调整终止条件中的参数

    def g_tf(self, t, x):
        # 根据 g_param 调整终止条件
        return 1 - 1.0 / (1 + tf.exp(self.g_param * t + tf.reduce_sum(x, 1, keepdims=True) / self.dim))
...
class CustomReactionDiffusion(ReactionDiffusion):
    """
    Customizable Multidimensional ReactionDiffusion PDE 
    with parameterized terminal condition g.
    """
    def __init__(self, eqn_config):
        super(CustomReactionDiffusion, self).__init__(eqn_config)
        self.g_param = eqn_config.get('g_param', 1.0)  # 动态参数，用于调整终止条件中的参数

    def g_tf(self, t, x):
        return 1 + self._kappa + tf.sin(self.lambd * tf.reduce_sum(x, 1, keepdims=True)) * self.g_param