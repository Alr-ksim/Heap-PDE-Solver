...
class BSDESolver(object):
    ...
    def record_solution(self, batch_size=1, final_loss=None):
        """记录t=0, t=T/2, t=T时的三元组，并记录g_param和最后的loss"""
        records = {
            "g_param": self.eqn_config.g_param,  # 记录当前g_param
            "final_loss": final_loss,  # 记录最后一轮训练的loss
            "data": []
        }

        # 采样输入数据
        dw, x = self.bsde.sample(batch_size)
        time_stamp = np.arange(0, self.eqn_config.num_time_interval) * self.bsde.delta_t
        mid_time_index = self.eqn_config.num_time_interval // 2  # 中间时间步的索引
    
        # 初始时刻 t = 0
        y = tf.ones([batch_size, 1], dtype=self.net_config.dtype) * self.y_init
        z = self.model.subnet[0](x[:, :, 0], training=False) / self.bsde.dim
        records["data"].append((0, x[:, :, 0].tolist(), y.numpy().tolist()))  # 记录 t=0 的三元组

        # 递推到 t = T/2
        for t in range(0, mid_time_index):
            y = y - self.bsde.delta_t * self.bsde.f_tf(time_stamp[t], x[:, :, t], y, z) + tf.reduce_sum(z * dw[:, :, t], 1, keepdims=True)
            z = self.model.subnet[t](x[:, :, t + 1], training=False) / self.bsde.dim
        records["data"].append((time_stamp[mid_time_index], x[:, :, mid_time_index].tolist(), y.numpy().tolist()))  # 记录 t=T/2 的三元组

        # 最后一步使用终止条件 g 来计算 u(T)
        u_terminal = self.bsde.g_tf(self.bsde.total_time, x[:, :, -1])
        records["data"].append((self.bsde.total_time, x[:, :, -1].tolist(), u_terminal.numpy().tolist()))  # 记录 t=T 的三元组
